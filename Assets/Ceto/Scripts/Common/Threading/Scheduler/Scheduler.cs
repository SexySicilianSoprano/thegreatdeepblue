...
namespace Ceto.Common.Threading.Scheduling
{
    ...
    /// Scheduler.
    /// </summary>
    public class Scheduler : IScheduler, ITaskScheduler
    {

        /// <summary>
		/// How many tasks were ran this update.
        /// </summary>
        public int tasksRanThisUpdate { get { return m_tasksRanThisUpdate; } }
        int m_tasksRanThisUpdate = 0;

		/// <summary>
		/// How many task were finished this update.
		/// </summary>
		public int TasksFinishedThisFrame { get { return m_tasksFinishedThisUpdate; } }
		int m_tasksFinishedThisUpdate;

		/// <summary>
		/// Max tasks that will be ran per update.
		/// </summary>
		public int MaxTasksPerFrame { get { return m_maxTasksPerUpdate; } }
		int m_maxTasksPerUpdate = 1;

		/// <summary>
		/// Max tasks to run per update.
		/// </summary>
		public int MaxFinishPerFrame { get { return m_maxFinishPerUpdate; } }
		int m_maxFinishPerUpdate = 1;
        ...
        /// <summary>
		/// A a exception thrown by a running task to be rethrown by the scheduler
        /// </summary>
		...
		/// <summary>
		/// Temporary list hold task that have ran.
		/// </summary>
		LinkedList<IThreadedTask> m_haveRan;

		/// <summary>
		/// Is the scheduler shutting down.
		/// </summary>
		...
		public Scheduler()
		{
			...
			m_coroutine = null;
			m_maxTasksPerUpdate = 100;
			m_maxFinishPerUpdate = 100;
		...
		public Scheduler(int maxTasksPerUpdate, int maxFinishPerUpdate, ICoroutine coroutine)
        {
			...
			m_maxTasksPerUpdate = Math.Max(1, maxTasksPerUpdate);
			m_maxFinishPerUpdate = Math.Max(1, maxFinishPerUpdate);
		...
		public void Update()
		{
			//Dont forget any multi-threaded or coroutine tasks must 
			//tell the scheduler when they are finished from their Run 
			//function by calling FinishedRunning().

			m_tasksRanThisUpdate = 0;
			m_tasksFinishedThisUpdate = 0;

			//clean up any tasks that have finished since last time
			//scheduler was updated.
			FinishTasks();

			int running = RunningTasks();

			for(int i = 0; i < m_maxTasksPerUpdate - running; i++)
			{
				...
				if(RunningTasks() == 0 && ScheduledTasks() == 0 && WaitingTasks() == 0) break;
			}
        ...
        void RunThreaded(object o)
        {
			...
			if (!(o is IThreadedTask)) {
                Throw(new InvalidCastException("Object is not a ITask"));
            }
            else
            {
        ...
        {
			if(m_tasksFinishedThisUpdate >= m_maxFinishPerUpdate) return;

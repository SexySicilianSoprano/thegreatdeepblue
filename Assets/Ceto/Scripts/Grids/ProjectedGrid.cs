...
namespace Ceto
{
	...
	public class ProjectedGrid : OceanGridBase
    {
		...
		static readonly bool DISABLE_BORDER = false;
        ...
        public class Grid
        {
			...
			public int groups;
			public IList<ScreenQuad> quads = new List<ScreenQuad>();
        ...
        void Update()
        {
			...
			try
			{
                ...
                {
                    if(oceanTopSideMat.renderQueue <= 2500)
                    {
		...
		void UpdateGrid(Grid grid)
		{
            ...
            int count = grid.quads.Count;
            for (int i = 0; i < count; i++)
			{
                    grid.quads[i].mesh.bounds = new Bounds(Vector3.zero, new Vector3(bigNumber, level + range, bigNumber));
			}

            //Update the shadow and reflection probe settings
            count = grid.topRenderer.Count;
            for (int i = 0; i < count; i++)
			{
            ...
            //This may casue some issues if changed so zero transform before rendering.
            //The game object should be hidden so it is unlikely to be modified but just incase.
		...
		/// </summary>
        void ApplyProjection(GameObject go, object arg)
        {

			try
			{

				if(!enabled || !(arg is ScreenQuad)) return;

				Camera cam = Camera.current;
                //ScreenQuad quad = (ScreenQuad)arg;
                ...
                if (!data.projection.updated)
				{
					...
					m_ocean.Projection.UpdateProjection(cam, m_ocean.ProjectSceneView);
        ...
        void CreateGrid(MESH_RESOLUTION meshRes)
        {
			...
			//There is currently a bug where when the mesh is recreated during run time it has some holes in it.
			//Disabling recreation on screen resize for now.
            ...
            //Debug.Log(Time.frameCount + " Creating grid width = " + screenWidth + " height = " + screenHeight);
            ...
            grid.quads = CreateScreenQuads(resolution, groups, screenWidth, screenHeight);
		
			//For each mesh make a gameobject for the top side and under side.
			foreach (ScreenQuad quad in grid.quads)
            {
				...
				if(oceanTopSideMat != null)
				{
					...
					GameObject top = new GameObject(Ocean.InstanceName + " TopSide Grid LOD: " + meshRes);
	                ...
	                willRender.AddAction(ApplyProjection, quad);
				...
				if(oceanUnderSideMat)
				{
					...
					GameObject under = new GameObject(Ocean.InstanceName + " UnderSide Grid LOD: " + meshRes);
		...
		void ClearGrid(Grid grid)
		{
			...
			if(grid.top != null)
			{
                ...
                for(int i = 0; i < count; i++)
                {
                    ...
                    Mesh mesh = grid.top[i].GetComponent<MeshFilter>().mesh;
                    Destroy(grid.top[i]);
                    Destroy(mesh);
                }
			...
			if(grid.under != null)
			{
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    Mesh mesh = grid.under[i].GetComponent<MeshFilter>().mesh;
			...
			if(grid.quads != null)
			{
                ...
                for (int i = 0; i < count; i++)
                {
                    if (grid.quads[i] == null) continue;

                    Destroy(grid.quads[i].mesh);
                }
		...
		/// </summary>
		IList<ScreenQuad> CreateScreenQuads(int resolution, int groupSize, int width, int height)
		{
			...
			List<ScreenQuad> quads = new List<ScreenQuad>();
			...
			for(int x = 0; x < numX; x++)
			{
				for(int y = 0; y < numY; y++)
				{
					...
					ScreenQuad quad = CreateQuad(numVertsX, numVertsY, ux, uy, w, h);
					quads.Add(quad);
		...
		/// </summary>
		public ScreenQuad CreateQuad(int numVertsX, int numVertsY, float ux, float uy, float w, float h)
		{
            ...
            mesh.name = Ocean.InstanceName + " Projected Grid Mesh";
            mesh.hideFlags = HideFlags.HideAndDontSave;

			ScreenQuad quad = new ScreenQuad();

			quad.mesh = mesh;
			quad.corners = new Vector2[4];

			Vector3 c0 = vertices[0 + 0 * numVertsX];
			Vector3 c1 = vertices[(numVertsX-1) + 0 * numVertsX];
			Vector3 c2 = vertices[(numVertsX-1) + (numVertsY-1) * numVertsX];
			Vector3 c3 = vertices[0 + (numVertsY-1) * numVertsX];

			quad.corners[0] = new Vector2(c0.x, c0.y);
			quad.corners[1] = new Vector2(c1.x, c1.y);
			quad.corners[2] = new Vector2(c2.x, c2.y);
			quad.corners[3] = new Vector2(c3.x, c3.y);

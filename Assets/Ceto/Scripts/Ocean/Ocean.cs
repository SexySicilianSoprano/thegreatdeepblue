...
namespace Ceto
{
    ...
    public class Ocean : MonoBehaviour
    {
        ...
        /// <summary>
        /// Returns a name with the instance hash.
        /// Used to name resources. The Makes it easy to tell
        /// if the resource has been release correctly in the editor.
        /// </summary>
        public static string InstanceName {  get { return "Ceto";  } }
        ...
        /// </summary>
        public const float MAX_SPECTRUM_WAVE_HEIGHT = 50.0f;

		/// <summary>
		/// The max wave height from the overlays.
		/// </summary>
		public const float MAX_OVERLAY_WAVE_HEIGHT = 30.0f;

		/// <summary>
		/// The max wave height. Any waves higher than this will be clamped.
		/// </summary>
		...
		/// <summary>
		/// Manager class for splashes.
		/// </summary>
		//public SplashManager SplashManager { get; private set; }
		...
		void Awake()
		{
            ...
            try
            {
				...
				//SplashManager = new SplashManager(splashPrefabs);
        ...
        void Update()
        {
			...
			try
			{
				...
				Shader.SetGlobalFloat("Ceto_MaxWaveHeight", MAX_WAVE_HEIGHT);
				...
				Shader.SetGlobalVector("Ceto_FoamTextureScale0", new Vector3(1.0f / foamParam0.x, 1.0f / foamParam0.y, foamParam0.z));

				Shader.SetGlobalTexture("Ceto_FoamTexture1", ((foamTexture1.tex != null) ? foamTexture1.tex : Texture2D.whiteTexture));
				Shader.SetGlobalVector("Ceto_FoamTextureScale1", new Vector3(1.0f / foamParam1.x, 1.0f / foamParam1.y, foamParam1.z));
		...
		{
            Debug.Log("<color=blue>Ceto (" + VERSION + ") Info:</color> " + msg);
		}
        ...
        public void RemoveCameraData(Camera cam)
        {
            ...
            {
                RTUtility.ReleaseAndDestroy(data.overlay.normal);
                RTUtility.ReleaseAndDestroy(data.overlay.height);
                RTUtility.ReleaseAndDestroy(data.overlay.foam);
                RTUtility.ReleaseAndDestroy(data.overlay.clip);

                data.overlay.normal = null;
                data.overlay.height = null;
                data.overlay.foam = null;
                data.overlay.clip = null;
            }
		...
		public void QueryWavesAsync(IEnumerable<WaveQuery> querys, Action<IEnumerable<WaveQuery>> callBack)
		{
			...
			if(enabled && Spectrum != null && !(Spectrum.DisplacementBuffer.IsGPU && Spectrum.DisableReadBack))
			{
        ...
        public void RenderWaveOverlays(GameObject go)
        {
			...
			try
			{
				...
				//If the projection for this camera has not been updated
				//this frame do it now. This will normally be done
				//by the projected grid.
				if(data.projection == null || !data.projection.updated)
				{
					Projection.UpdateProjection(cam, ProjectSceneView);
				...
				if(OverlayManager.HasNormalOverlay)
                    CreateOverlay("Normal", cam, normalOverlaySize, RenderTextureFormat.ARGBHalf, true, ref data.overlay.normal);
				if(OverlayManager.HasHeightOverlay)
                    CreateOverlay("Height", cam, heightOverlaySize, RenderTextureFormat.RGHalf, true, ref data.overlay.height);
				if(OverlayManager.HasFoamOverlay)
                    CreateOverlay("Foam", cam, foamOverlaySize, RenderTextureFormat.ARGB32, false, ref data.overlay.foam);

                if (OverlayManager.HasClipOverlay)
                {
                    RenderTextureFormat format = RenderTextureFormat.R8;

                    //R8 not supported on some systems
                    if (!SystemInfo.SupportsRenderTextureFormat(format))
                        format = RenderTextureFormat.RHalf;

                    CreateOverlay("Clip", cam, clipOverlaySize, format, true, ref data.overlay.clip);
                }

				//Render the overlays
				OverlayManager.RenderWaveOverlays(cam, data.overlay);

				//If map has been created apply it to shaders
				if(data.overlay.normal != null)
					Shader.SetGlobalTexture("Ceto_Overlay_NormalMap", data.overlay.normal);
				if(data.overlay.height != null)
					Shader.SetGlobalTexture("Ceto_Overlay_HeightMap", data.overlay.height);
				if(data.overlay.foam != null)
					Shader.SetGlobalTexture("Ceto_Overlay_FoamMap", data.overlay.foam);
				if(data.overlay.clip != null)
					Shader.SetGlobalTexture("Ceto_Overlay_ClipMap", data.overlay.clip);
        ...
        /// by the projected grid.
        /// </summary>
        public void UpdateProjection(GameObject go)
        {
        ...
        /// <summary>
        /// Create a overlay with these settings.
        /// </summary>
		void CreateOverlay(string name, Camera cam, OVERLAY_MAP_SIZE size, RenderTextureFormat format, bool isLinear, ref RenderTexture map)
		{
			...
			if(map == null || map.width != w || map.height != h)
			{
                ...
                map.name = InstanceName + " Overlay " + name + " Buffer: " + cam.name;
			}

...
namespace Ceto
{
	...
	public abstract class WaveSpectrumBufferCPU : WaveSpectrumBuffer
	{
		...
		static readonly bool DISABLE_INIT_MULTITHREADING = false;
		static readonly bool DISABLE_FOURIER_MULTITHREADING = false;
        ...
        /// </summary>
		protected ThreadedTask m_initTask;
		...
		public WaveSpectrumBufferCPU(int size, int numBuffers, Scheduler scheduler)
		{
			...
			WTable = CreateWTable(size, inverseGridSizes);
		...
		Buffer CreateBuffer(int size)
		{
            ...
            buffer.map.name = Ocean.InstanceName + " Wave Spectrum CPU Buffer";
		...
		public override void Run(WaveSpectrumCondition condition, float time)
		{
			...
			if(DISABLE_INIT_MULTITHREADING)
			{
			...
			for(int i = 0; i < numBuffers; i++)
			{
				...
				FourierTask task = new FourierTask(this, m_fourier, i);

				if(DISABLE_INIT_MULTITHREADING && DISABLE_FOURIER_MULTITHREADING)
				{
                    //Multithreading disabled, run now on this thread.
					task.Start();
					task.Run();
			...
			if(!DISABLE_INIT_MULTITHREADING)
			{
				...
				//before there over tasks have been added and a 

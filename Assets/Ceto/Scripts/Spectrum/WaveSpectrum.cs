...
namespace Ceto
{
	...
	public class WaveSpectrum : WaveSpectrumBase
	{
		...
		public override Vector2 GridScale { get { return new Vector2(gridScale, gridScale); } }
		...
		/// Gets the grid sizes.
		/// </summary>
		public override Vector4 GridSizes { get { return GRID_SIZES; } }
		...
		void Start()
		{
			...
			try
			{
                ...
                m_queryScaling.gridSizes = GridSizes;
                m_queryScaling.gridScale = GridScale;
                m_queryScaling.choppyness = m_choppyness;
		...
		void Update()
		{
			...
			try
			{
				...
				m_choppyness = CHOPPYNESS * Mathf.Clamp(choppyness, 0.0f, MAX_CHOPPYNESS);

				Shader.SetGlobalVector("Ceto_GridSizes", GridSizes);
				Shader.SetGlobalVector("Ceto_GridScale", GridScale);
				Shader.SetGlobalVector("Ceto_Choppyness", m_choppyness);
		...
		void GenerateSlopes(float time)
		{
            ...
            int enabledBuffers = m_slopeBuffer.EnabledBuffers();

            if (disableSlopes && enabledBuffers != 0)
        ...
        void GenerateDisplacement(float time)
		{
            ...
            int enabledBuffers = m_displacementBuffer.EnabledBuffers();

            if (disableDisplacements && enabledBuffers != 0)
            ...
            if (!disableDisplacements && choppyness == 0.0f && m_displacementBuffer.IsEnabledBuffer(1) && m_displacementBuffer.IsEnabledBuffer(2))
			{
			...
			else if(m_displacementBuffer.Done)
			{
				...
				if(!m_displacementBuffer.BeenSampled)
				{
					...
					Graphics.Blit(null, m_displacementMaps[0], m_displacementCopyMat, 0);
		...
		void FindRanges()
		{
			...
			if(disableReadBack && DisplacementBuffer.IsGPU)
			{
				...
				MaxDisplacement = new Vector2(0.0f, Ocean.MAX_WAVE_HEIGHT);
			}
        ...
        void GenerateFoam(float time)
		{
            ...
            int enabledBuffers = m_jacobianBuffer.EnabledBuffers();

            if ((disableFoam || foamAmount == 0.0f) && enabledBuffers != 0)
			...
			else
			{
				...
				{
                    Shader.SetGlobalFloat("Ceto_FoamAmount", foamAmount);
				...
				if(!m_jacobianBuffer.BeenSampled)
				{
					...
					m_foamCopyMat.SetTexture("Ceto_JacobianXXBuffer", m_jacobianBuffer.GetTexture(0));
					m_foamCopyMat.SetTexture("Ceto_JacobianYYBuffer", m_jacobianBuffer.GetTexture(1));
					m_foamCopyMat.SetTexture("Ceto_JacobianXYBuffer", m_jacobianBuffer.GetTexture(2));
					m_foamCopyMat.SetTexture("Ceto_HeightBuffer", m_displacementBuffer.GetTexture(0));
					m_foamCopyMat.SetVector("Ceto_FoamChoppyness", CHOPPYNESS);
					m_foamCopyMat.SetFloat("Ceto_FoamCoverage", foamCoverage);

					//Graphics.Blit(null, m_foamMaps[0], m_foamCopyMat, 0); //Unused atm.
					Graphics.Blit(null, m_foamMaps[1], m_foamCopyMat, 1);
                ...
                Shader.SetGlobalTexture("Ceto_FoamMap1", m_foamMaps[1]);
			}
		...
		public override void QueryWaves(WaveQuery query)
		{
			...
			if(disableReadBack && DisplacementBuffer.IsGPU) return;
            ...
            DisplacementBuffer.QueryWaves(query, m_queryScaling);
        ...
        void CreateBuffers()
        {
            ...
            if (m_bufferSettings.beenCreated)
            {
                ...
                else
                {
                    ...
                    //Settings change and safe to recreate buffers.
            ...
            if (isCpu)
                ...
                m_displacementBuffer = new DisplacementBufferGPU(size, initDisplacementSdr, fourierSdr);

            m_slopeBuffer = new WaveSpectrumBufferGPU(size, initSlopeSdr, fourierSdr, 2);
            m_jacobianBuffer = new WaveSpectrumBufferGPU(size, initJacobianSdr, fourierSdr, 3);
            ...
            m_foamMaps = new RenderTexture[2];
		...
		/// <summary>
		/// Creates the maps to hold the displacement, slope and foam.
		/// </summary>
		void CreateMap(ref RenderTexture map, string name, RenderTextureFormat format, int size, int ansio)
		{
            ...
            map.name = Ocean.InstanceName + " Wave Spectrum " + name + " Texture";
			map.Create();
		...
		void CreateConditions()
		{
            ...
            int size = m_bufferSettings.size;
            var key = new WaveSpectrumConditionKey(size, windSpeed, m_ocean.windDir, waveAge);
			...
			if(m_conditions[0] == null)
			{
                ...
                {
                    m_conditions[0] = new WaveSpectrumCondition(size, windSpeed, m_ocean.windDir, waveAge);

                    CreateSpectrumTask task = new CreateSpectrumTask(m_conditions[0]);
			...
			else if(m_conditions[1] == null && m_conditions[0].Done && key != m_conditions[0].Key)
			{
                ...
                else
                {
                    ...
                    m_conditions[1] = new WaveSpectrumCondition(size, windSpeed, m_ocean.windDir, waveAge);

                    //This can take quite a while to do
                    //so run on a separate thread using a task. 
                    CreateSpectrumTask task = new CreateSpectrumTask(m_conditions[1]);
		...
		public void CreateAndCacheCondition(int fourierSize, float windSpeed, float windDir, float waveAge)
		{
			...
			var condition = new WaveSpectrumCondition(fourierSize, windSpeed, windDir, waveAge);

			if(m_conditionCache.ContainsKey(condition.Key)) return;
			
			CreateSpectrumTask task = new CreateSpectrumTask(condition);

...
inline fixed4 OceanBRDFLight(SurfaceOutputOcean s, half3 viewDir, UnityLight light)
{
	...
	V.y *= -1.0;
		...
		half3 spec = ReflectedSunRadianceNice(V, N, light.dir, s.Fresnel) * s.SpecularStr;
	#else
		half3 spec = ReflectedSunRadianceFast(V, N, light.dir, s.Fresnel) * s.SpecularStr;
	#endif
	
	fixed diff = max (0, dot (DN, light.dir));

	#ifdef CETO_NO_DIFFUSE_IN_REFLECTIONS
	...
	half spec = pow (nh, 128.0)  * s.SpecularStr;
...
inline fixed4 LightingOceanBRDF(SurfaceOutputOcean s, half3 viewDir, UnityGI gi)
{
	...
	fixed4 c = OceanBRDFLight (s, viewDir, gi.light);
		...
		#ifdef CETO_NO_DIFFUSE_IN_REFLECTIONS
			c.rgb += s.Albedo * gi.indirect.diffuse * s.IndirectStr * (1.0 - s.Fresnel);
		#else
			c.rgb += s.Albedo * gi.indirect.diffuse * s.IndirectStr;
		#endif
	#endif

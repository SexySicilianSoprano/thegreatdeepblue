...
fixed3 OceanWithFoamColor(float3 worldPos, fixed3 oceanCol, fixed4 foam)
{
	...
	fixed foamTexture = 0.0;
   	...
   	fixed foamAmount = foam.x + foam.y;
...
float4 SampleOceanDepth(float2 screenUV, half3 distortion)
{
	...
	oceanDepths = tex2D(Ceto_OceanDepth, screenUV);

	//unnormalize.
	oceanDepths.x *= Ceto_MaxDepthDist;
	oceanDepths.y = oceanDepths.y * _ProjectionParams.z / Ceto_MaxDepthDist;
...
* y = dist from cam pos normalized to far plane.
* z = linear depth value.
*/
...
*/
fixed3 AboveRefractionColor(float2 grabUV, half3 distortion, float3 surfacePos, float depthBlend)
{
	
	fixed3 grab = SampleRefractionGrab(grabUV, distortion).rgb * Ceto_RefractionIntensity;
	
	fixed3 col = grab * Ceto_AbsTint * exp(-Ceto_AbsCof.rgb * depthBlend * Ceto_MaxDepthDist * Ceto_AbsCof.a);
...
*/
fixed3 AddAboveInscatter(fixed3 col, float depthBlend)
{
	...
	inscatterScale.y = saturate(1.0-exp(-depthBlend * Ceto_AboveInscatterScale));
	inscatterScale.z = saturate(1.0-exp(-depthBlend * depthBlend * Ceto_AboveInscatterScale));
...
*/
fixed3 AddBelowInscatter(fixed3 col, float depthBlend)
{
	//There are 3 methods used to apply the inscatter.
	half3 inscatterScale;
	inscatterScale.x = saturate(depthBlend * Ceto_BelowInscatterScale);
	inscatterScale.y = saturate(1.0-exp(-depthBlend * Ceto_BelowInscatterScale));
	inscatterScale.z = saturate(1.0-exp(-depthBlend * depthBlend * Ceto_BelowInscatterScale));
...
fixed3 OceanColorFromAbove(half3 N, float4 screenUV, float3 surfacePos, float surfaceDepth, float dist)
{
...
#ifdef CETO_USE_OCEAN_DEPTHS_BUFFER
	float depth = SampleOceanDepth(screenUV.xy, distortion).y;
#else
	...
	//oceanDepth.x is the distance from surface by world y where oceanDepth.y
	//is distance from surface by camera z. Blend between them so user
	//can choose which method looks best. Need to times y by z_scale as it has a much
	//stronger look and decreasing it better matches the x value.
	float z_scale = 0.5;

	float depthBlend = lerp(oceanDepth.x, oceanDepth.y * z_scale, Ceto_DepthBlend);
...
*/ 
fixed3 UnderWaterColor(fixed3 belowColor, float depth)
{
	...
	float dist = clamp(depth * _ProjectionParams.z, 0, Ceto_MaxDepthDist);

	col = belowColor;
	
	col *= Ceto_BelowTint * exp(-Ceto_BelowCof.rgb * dist * Ceto_BelowCof.a);
	
	col = AddBelowInscatter(col, depth);

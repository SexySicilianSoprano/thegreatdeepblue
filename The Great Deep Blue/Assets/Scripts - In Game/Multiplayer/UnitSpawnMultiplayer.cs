...
public class UnitSpawnMultiplayer : NetworkBehaviour {
    ...
    public void Start()
    {
        NetworkIdentity identity = GameObject.Find("Player1").GetComponent<NetworkIdentity>();
	...
	public void RpcLog(int spawnUnit) {
        ...
        Debug.Log(netId + " up in this bitch");
	...
	public void ServerInitialize(int spawnUnit) {
		...
		//Player unit
		GameObject obj = MonoBehaviour.Instantiate(spawnPrefab[spawnUnit], GetComponent<UnitSpawner>().m_SpawnerPos, GetComponent<UnitSpawner>().m_Spawner.rotation) as GameObject;
